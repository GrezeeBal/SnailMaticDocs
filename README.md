...
При клике на кнопку **«Добавить профиль»** добавляется профиль с указанным названием. С помощью ПКМ по созданному профилю ему можно изменить название или удалить его. Чтобы добавлять профили через проводник во время игры нужно - перезагружать биндер (`/sm.reload`). Профили находятся по адресу `C:\Users\user\Documents\GTA San Andreas User Files\SAMP\SnailMatic\profiles`
...
## 10.3. Функциональные диалоги/отправка строк бинда по отдельности

> [Небольшой гайд по созданию функциональных диалогов](https://vk.com/@scriptpatrol-gaid-po-sozdaniu-bindov?anchor=sozda-m-svoi-dialogovye-okna)
