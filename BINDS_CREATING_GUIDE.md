...
**Но!** Это поле слишком маленькое и длинное — это всего лишь одна строка, которую отправит биндер. Нам нужно вставить сразу несколько строк, поэтому мы поменяем вид этой строки на многострочный:

![image](https://user-images.githubusercontent.com/71496296/152537718-2926b803-29b8-4dd6-b960-fa3e8a7fcaa4.png)

Теперь наш редактор выглядит вот так:
...
Далее настроим задержку между отправками этих строк. В многострочном виде это сделать не получится, поэтому мы изменим его обратно на однострочный вариант:
...
Биндер автоматически разбил все строки на отдельные части, и теперь перейдем к добавлению задержек. **Важно!** Без задержек все сообщения отправятся моментально — это превратится во **флуд** и сервер будет **блокировать** эти отыгровки. Как избежать? — Выставить задержку у каждой строки:

![image](https://user-images.githubusercontent.com/71496296/152537832-64a3c59b-d2e4-487b-b018-9c96e6c87003.png)

Задержка в SnailMatic выставляется в **миллисекундах** (мсек). 1 секунда = 1000 мсек. Для большинства серверов подойдет задержка в 2 секунды, что равно 2000 мсек. Для каждого сообщения делаем задержку в 2000 мсек:

![image](https://user-images.githubusercontent.com/71496296/152537882-07551fa8-8793-4629-89d3-d1add49339cf.png)

Обрати внимание, что на последней строке мы не выставили задержку, почему? — выставляя задержку после одного сообщения, мы указываем биндеру, сколько ему нужно подождать перед отправкой следующего сообщения: 
...
Мы добавим переменные в следующую строку:
...
Дальше сделаем автоматический ввод ID игрока, которого нужно вылечить. Один из способов, как биндер распознает нужного нам игрока — это приблизиться к этому игроку, чтобы он стал самым ближайшим для тебя. В этом случае мы будем использовать переменную `$closestid$` — эта переменная впишет ID самого ближайшего к тебе игрока. Вставим её после команды `/heal`, чтобы получилось `/heal $closestid$`:

![image](https://user-images.githubusercontent.com/71496296/152538132-280f3bb4-d60b-4a94-a8ce-58bc245a9e08.png)

Под конец нам нужно установить активацию для этого бинда и сделать для него название. Назовем этот бинд «**Лечение**»
...
Далее в начале смены нам нужно просто вписать в чат команду `/unit` и наш юнит, как биндер создаст переменную `$unit$`, содержащую наш указанный юнит.
...
Чтобы это регулярное выражение заработало, нам нужно включить [Lua Pattern.](https://github.com/GrezeeBal/SnailMaticDocs/blob/main/FUNCTIONAL_VARIABLES_GUIDE.md#chatwordsex) 

> по ссылке выше, в описании переменной _@chatwordsex_ есть небольшой гайд по составлению таких регулярок
...
При появлении СМС наш бинд будет автоматически активироваться и отправляться на сервер сообщение `/sms [номер телефона]`, но нам нужно писать ответное сообщение на входящее СМС —

### Этап 3

— для этого изменим отправку строки с командой `/sms [номер телефона]` на «**Написать в чат**»

![image](https://user-images.githubusercontent.com/71496296/152541140-c64ca7c7-b84e-4827-9390-90d107393bf2.png)

Это даст указание биндеру при активации бинда открыть чат и вписать туда заготовку команды `/sms [номер телефона]` с автоматически подставленным номером телефона.

![image](https://user-images.githubusercontent.com/71496296/152541183-bb065823-7f40-4603-be5f-fb9dad8ed233.png)

Небольшая проблема: курсор ввода стоит вплотную к номеру телефона — нам что, нужно самим нажимать пробел, чтобы вписать текст? — Нет, просто добавим этот пробел в строку в бинде

![image](https://user-images.githubusercontent.com/71496296/152541203-54a8785b-4e1f-4259-8fdf-c0cd5a080a56.png)

При появлении СМС в чате: `SMS: Текст | Отправитель: Nick_Name [т.number]` будет автоматически открываться чат с заготовленной командой /sms и номером телефона — это не удобно и будет часто мешать. Как решить? —

### Этап 4

— Укажем биндеру, чтобы он спрашивал у нас, активировать этот бинд или нет. Для этого есть галочка в окне редактора биндов «**Требовать подтверждение активации**»
...
_Если бинды с отыгровками находятся не в первой папке, а в любой другой — в параметр переменной `@bindstart` нужно будет дописать название папки, как вот например тут:_
