...
- Новые переменные
  - `@bindstate` - текущее состояние указанного в параметре бинда: "**Запущен**", "**На паузе**", "**Не запущен**"
  - `@dialoginput` - создает кастомное диалоговое окно, которое возвращает вписанное в него значение
    - `@dialoginput(Введите ваш ID)@` - при вводе ID (текст может быть любой) - переменная вернет это же значение.

    <img width="263" height="166" alt="image" src="https://github.com/user-attachments/assets/d78d3315-417c-4fc4-9dc7-cda5f61fe12c" />
    
  - `@dialoglist` - создает кастомное диалоговое окно с разными вариантами выбора. В зависимости от выбранного варианта возвращает индекс пункта
    - `@dialoglist(Да;Нет;Отменить)@` - при выборе варианта `Да` - переменная вернёт `0`. При выборе `Нет` - `1` и так далее.
      
    <img width="441" height="303" alt="image" src="https://github.com/user-attachments/assets/e052acf9-efff-4515-a495-95ff24203eb4" />
    
- Возможность добавить папку в селектор.
  - <img width="452" height="195" alt="image" src="https://github.com/user-attachments/assets/056b049f-97d2-46ec-b05c-58ef04386779" />
  - <img width="323" height="328" alt="image" src="https://github.com/user-attachments/assets/60dbff56-d57c-4b1a-bdda-8a06f5562548" />


### Обновлено

- Обновили переменную `@random`. Теперь в неё можно указать несколько вариантов текста.
  - `@random(Привет;Здраствуйте;Здарова)` - напишет либо "Привет", либо "Здраствуйте", либо "Здарова"
- Привязали появление селектора к центру экрана

### Исправлено

- Исправили баг, при котором переменные `@bind...` (_@bindstart, @bindstop, @bindactivate..._) выполнялись в любом случае, независимо от правильности условия в `@ifandor`
- Исправили баг, при котором `@dialogitem` мог выбрать неправильный пункт в некоторых диалогах
- В условии `@ifandor` теперь работают логические операторы `and` `or`
- Исправили отправку "Написать в чат" и "Написать в чат и закрыть его" на некоторых лаунчерах.
  - **Фикс нужно активировать самостоятельно** с помощью галочки в настройках "Альтернативная отправка в чат"
- Теперь звук уведомлений от биндера можно включить даже при беспалевном режиме
- Исправили проблему, при которой нельзя было вставить большой текст в редактор бинда в режиме блокнота
- Теперь кириллица в шпаргалке копируется корректно
- Прочие мелкие изменения

</br>

## Выпущено: `13/08/2024`
SnailMatic 1.9 Achatina

### Добавлено

- Added English localization. Can be switched in binder settings.
- Английская локализация для биндера. Изменить можно в настройках биндера.
- Создав текстовый файл под названием `nointernet.txt` в папке _\Документы\GTA San Andreas User Files\SAMP\SnailMatic_, биндер не будет посылать запросы на свой хост
- Новые условия активации бинда:
  - Не запускать при активном курсоре (полезно, если открыт другой скрипт с ImGui или серверный текстдрав)
  - Не запускать при открытом TAB
- В виджеты (HUD) теперь можно добавлять картинки из интернета. Работает так же, как и с добавлением обычной картинки, но вместо локального пути на компьютере указывается URL-адрес:
  - `#img(http://site.com/img.png)` - картинка по пути `site.com/img.png` с оригинальным размером

### Обновлено

- Виджеты (HUD) теперь скрываются при открытии TAB
- Обновили переменные `$closest...`. Теперь они не возвращают игроков в твоей машине.
- Обновили переменную `@ifandor`. Добавили для неё экранирование символов `?` и `:`. Подробнее читай в подсказке к переменной
- Изменили некоторые команды биндера (при этом старые так же будут работать):
  - `/sm.bind` на `/sm.bindstart`
  - `/sm.stop` на `/sm.bindstop`
  - `/sm.spur` на `/sm.note`
- Улучшили систему ввода ID для выбора таргета. Теперь она работает быстрее.
- Прочие мелкие изменения

### Исправлено

- Исправили краш при использовании переменой `$dialogclose$`
- Виджеты теперь не блокируют передвижение при прицеливании на ПКМ (если виджет находился по середине экрана)
...
- Новые переменные:
  ...
  - `$thisbind$` - пишет имя и папку запущенного бинда. Полезно для переменных, которые взаимодействуют с биндами. Например `@bindstop($thisbind$)@` остановит бинд, в котором была вызвана эта переменная
  ...
  - `@hudactivate()@` - включает виджет по его номеру/названию. Например @hud(1)@ или @hud(HUD 1)@ включит виджет под номером 1
  - `@huddeactivate()@` - выключает виджет по его номеру/названию. Например @hud(1)@ или @hud(HUD 1)@ выключит виджет под номером 1
...
- Теперь, чтобы поменять фон интерфейса биндера нужно закинуть картинку с названием **background.png** в _\Documents\GTA San Andreas User Files\SAMP\SnailMatic_ (рекомендуемый размер 640х654)
