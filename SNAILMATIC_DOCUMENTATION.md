...
*скачивать библиотеки отдельно не нужно, всё идет в архиве с биндером ([1.Установка]((https://github.com/GrezeeBal/SnailMaticDocs/blob/main/SNAILMATIC_DOCUMENTATION.md#1-%D1%83%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%BA%D0%B0)))*
...
В случае, если биндер не запустился/не работает — установи все библиотеки по ссылкам выше (2. [Требования](https://github.com/GrezeeBal/SnailMaticDocs/blob/main/SNAILMATIC_DOCUMENTATION.md#%D1%82%D1%80%D0%B5%D0%B1%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D1%8F)). Для подробной информации об ошибках - открой текстовый документ *...\moonloader\moonloader.log* и в строчке `(error) SnailMatic:` - прочитай ошибку.

Так же некоторые ошибки и их решения описаны здесь:
> 12. [Известные ошибки и их решения](#12-известные-ошибки-и-их-решения)

В ином случае обратись в [тех. поддержку (vk.com)](https://vk.com/scriptpatrol), приложив в сообщении файл `moonloader.log` ( `\GTA San Andreas\moonloader\moonloader.log` )

> Тех. поддержка закрыта с марта 2022. В группе есть беседа с другими пользователями биндера, которые могут помочь
...
<details>
  <summary markdown="span">Пример, с добавлением подсказки к функ. переменной:</summary>
  
```Lua
sm.registerFunctionalVariable("chatwordsl", "Слово из определенного сообщения в чате", function(param)
  local line, n = param:match("(%d+);(%d+)")
  return string.split(select(1, sampGetChatString(tonumber(line) or 99)),' ', true)[tonumber(n) or 1]
  end, [[@chatwordsl(99;1)@ - первое слово из последнего сообщения
...
`Решение`: Ошибка возникает из-за старой видеокарты, которая не в состоянии обработать большой атлас шрифтов. Для фикса установи вместо основного файла биндера один из файлов `snailmatic videocard 512x.luac` с [этой ссылки](https://drive.google.com/drive/u/0/folders/1tbH2SZpHWJtNs72FrFZSsr3JKkSLKje1). 
Минус этих версий в том, что при обновлении биндера они будут перекачиваться на основную версию, и тебе прийдется заново переустанавливать этот фикс (все настройки и профили сохранятся). Так же в них не будут работать иконки в конструкторе HUD и шпаргалке
