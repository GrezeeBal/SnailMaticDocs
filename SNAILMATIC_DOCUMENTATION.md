...
В случае, если биндер не запустился/не работает — установи все библиотеки по ссылкам выше ([2. Требования](#2-требования-для-биндера-и-его-запуск). Для подробной информации об ошибках - открой текстовый документ *...\moonloader\moonloader.log* и в строчке `(error) SnailMatic:` - прочитай ошибку.
