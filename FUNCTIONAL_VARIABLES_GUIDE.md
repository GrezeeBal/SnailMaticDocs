...
- `@bindstart($thisbind$)@` - запустит бинд, в котором была вызвана эта переменная. Полезно для создания флудеров.
...
<details><summary>Показать описание</summary>
  ...
  Эта переменная возвращает нужный текст из определенной строки в чате (либо из последней строки, либо по её номеру).
  ...
  Чтобы лучше представлять, что нужно писать в параметр переменной - мы разобъем эту строку по объектам. Каждый объект разделен между собой пробелами.
  ...
  — эти объекты, а именно их номера и пишутся в параметр переменной `@chatwords`, на основе чего эта переменная вернёт текст из этой строки

  ### Параметры этой переменной:

  ```
  @chatwords(4+)@ - подставит вместо себя весь текст, идущий после 4 объекта
  @chatwords(6-)@ - подставит вместо себя весь текст, идущий до 6 объекта
  @chatwords(2-4)@ - подставит вместо себя весь текст в промежутке от 2 до 4 объекта
  @chatwords(2)@ - подставит вместо себя 2 объект
  @chatwords(98;5)@ - пятый объект из предпоследнего сообщения
  @chatwords(98;4+)@ - весь текст, после четвертого объекта из предпоследнего сообщения
  ```

  Например, переменная `@chatwords(5+)@` подставит вместо себя весь текст, идущий после **пятого** объекта (включительно). Результат:

  ![chatwords](https://user-images.githubusercontent.com/71496296/152642275-9d618205-b755-4b11-baa9-ab7c143a78d3.gif)

  Или, например, переменная `@chatwords(12)@` подставит вместо себя объекта под порядковым номером **12**. Результат:
...
<details><summary>Показать описание</summary>
  ...
  Процесс создания регулярки выглядит так: 
  
  1) сначала мы подставляем вместо переменных символьные классы

  ![chatwordsex](https://user-images.githubusercontent.com/71496296/152642450-73cc9765-ceaf-4e4e-8fdb-7dce732f251f.gif)

  2) экранируем "магические" символы Луа паттерна `( ) . % + - * ? [ ] { } ^ $`, которые являются частью оригинального сообщения

  ![chatwordsex2](https://github.com/GrezeeBal/SnailMaticDocs/assets/71496296/9a53eb59-4d81-4eb8-a559-07068060e16c)

  3) затем нужные нам символьные классы (это имя, фамилия и номер) мы оборачиваем в скобки `()` (эти символы экранировать не нужно, так как мы их добавляем сами)

  ![chatwordsex3](https://github.com/GrezeeBal/SnailMaticDocs/assets/71496296/94018ea0-7c79-4e98-8860-d46b4d94e7e3)

  4) включаем галочку [Lua Pattern](https://www.blast.hk/threads/62661/), чтобы регулярное выражение заработало. (по гиперссылке можешь узнать о регулярных выражениях больше)

  ![image](https://github.com/GrezeeBal/SnailMaticDocs/assets/71496296/9de9a28e-cff7-4809-9b04-7e63e2f1e6dd)
...
<details><summary>Показать описание</summary>
  ...
  > пишет текст из диалога по его номеру (чтобы воспользоваться интерактивной подсказкой нужно открыть диалог)

  Например, мне нужно доставать из диалога динамически меняющийся текст "Игровое время `00:00`":

  ![dialogtext](https://github.com/GrezeeBal/SnailMaticDocs/assets/71496296/d3a2b791-7c5a-4bc6-a2e7-1705ed1cb4f9)

  Для этого нужно открыть этот диалог и навести мышку на интерактивную подсказку (плюсик) у названия `@dialogtext` в окне переменных.

  ![dialogtext2](https://github.com/GrezeeBal/SnailMaticDocs/assets/71496296/166dbd9d-a630-493c-a7ff-4831c5eb71bd)

  При копировании нужного мне текста (в моём случае `00:00`) - биндер автоматически скопирует переменную с уже вставленным номером текста (в моём случае `@dialogtext(69)@`)

  Теперь, если этот диалог открыт, и, например, текст `00:00` изменился на `12:00` - `@dialogtext(69)@` вернёт текст `12:00`
...
<details><summary>Показать описание</summary>
  ...
  > Важно! Когда нужно ставить кавычки в условиях:
  > 
  > **В тексте есть цифры и буквы** — нужно использовать кавычки
  > 
  > **В тексте только цифры** — не нужно использовать кавычки
...
<details><summary>Показать описание</summary>
  ...
  С помощью этой переменной можно вытаскивать параметры (аргументы) из введенной команды.
  
  - Эта переменная работает только через бинд и только с помощью активации по команде.
  ...
  > 
  > [Еще второй пример с использованием этой переменной](https://github.com/GrezeeBal/SnailMaticDocs/blob/main/BINDS_CREATING_GUIDE.md#4-%D1%81%D0%BE%D0%B7%D0%B4%D0%B0%D1%91%D0%BC-%D1%81%D0%B2%D0%BE%D0%B8-%D0%BF%D0%B5%D1%80%D0%B5%D0%BC%D0%B5%D0%BD%D0%BD%D1%8B%D0%B5)
...
<details><summary>Показать описание</summary>
  ...
  > Переключает состояние виджета, указанного в параметре по его номеру/названию (вкл/выкл)

  ![hud](https://github.com/GrezeeBal/SnailMaticDocs/assets/71496296/60e30297-ecb9-4ce6-b119-5b7ca4bf9113)

  `@hud(1)@` или `@hud(Рендер оружия)@` - переключит состояние худа под номером 1 (название "Рендер оружия") (включит, либо выключит его)
...
<details><summary>Показать описание</summary>
  ...
  > Включает виджет, указанный в параметре по его номеру/названию

  ![hud](https://github.com/GrezeeBal/SnailMaticDocs/assets/71496296/60e30297-ecb9-4ce6-b119-5b7ca4bf9113)

  `@hudactivate(1)@` или `@hudactivate(Рендер оружия)@` - включит худ под номером 1 (название "Рендер оружия")
...
<details><summary>Показать описание</summary>
  ...
  > Выключает виджет, указанный в параметре по его номеру/названию

  ![hud](https://github.com/GrezeeBal/SnailMaticDocs/assets/71496296/60e30297-ecb9-4ce6-b119-5b7ca4bf9113)

  `@huddeactivate(1)@` или `@huddeactivate(Рендер оружия)@` - выключит худ под номером 1 (название "Рендер оружия")
